...
P_Rain_given_Umbrella_False = (P_Rain['True'] * P_Umbrella_given_Rain[(
    'True', 'False')]) / P_Umbrella_given_Rain[('False', 'False')]
...
P_Rain_Umbrella_False = P_Rain['True'] * \
    P_Umbrella_given_Rain[('True', 'False')]
print(f"c. P(Rain=True, Umbrella=False) = {P_Rain_Umbrella_False:.2f}")

# d)P(Rain = False, Umbrella = True)
P_NoRain_Umbrella_True = P_Rain['False'] * \
    P_Umbrella_given_Rain[('False', 'True')]
print(f"d. P(Rain=False, Umbrella=True) = {P_NoRain_Umbrella_True:.2f}")

# e)P(Rain = False, Umbrella = False)
P_NoRain_Umbrella_False = P_Rain['False'] * \
    P_Umbrella_given_Rain[('False', 'False')]
...
P_Umbrella_given_NoRain = (
    P_Rain['False'] * P_Umbrella_given_Rain[('False', 'True')]) / P_Rain['False']
print(f"f. P(Umbrella=True | Rain=False) = {P_Umbrella_given_NoRain:.2f}")

# g)P(Grass = True | Umbrella = True)
P_Grass_True_Umbrella_True = P_Grass_given_Umbrella[(
    'True', 'True')] * P_Umbrella_given_Rain[('True', 'True')]
print(f"g. P(Grass=True | Umbrella=True) = {P_Grass_True_Umbrella_True:.2f}")

# h)P(Grass = False | Umbrella = False)
P_Grass_False_Umbrella_False = P_Grass_given_Umbrella[(
    'False', 'False')] * P_Umbrella_given_Rain[('False', 'False')]
print(f"h. P(Grass=F | Umbrella=F) = {P_Grass_False_Umbrella_False:.2f}")

# i) Determine the probability of Grass being False given that Umbrella is False.
P_Grass_False_Umbrella_False = P_Grass_given_Umbrella[(
    'False', 'False')] * P_Umbrella_given_Rain[('False', 'False')]
print(f"i. P(Grass=F | Umbrella=F) = {P_Grass_False_Umbrella_False:.2f}")

# j)Suppose you observe that the Grass is True. What is the probability that it is raining?
# we will use use Bayes' Theorem
P_Rain_Given_Grass_True = (
    P_Rain['True'] * P_Grass_given_Umbrella[('True', 'True')]) / P_Grass_True_Umbrella_True
print(f"j. P(Rain=True | Grass=True) = {P_Rain_Given_Grass_True:.2f}")
